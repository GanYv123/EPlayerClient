    ...
    , helper(this)
{
    ...
    int currentScreenWidth = screenGeometry.width();
    int currentScreenHeight = screenGeometry.height();
    ...
    connect(&info, SIGNAL(button_clicked()), this, SLOT(slot_connect_clicked()));
    ...
    connect(net, SIGNAL(finished(QNetworkReply*)), this, SLOT(slots_login_request_finshed(QNetworkReply*)));
...
void Widget::timerEvent(QTimerEvent* event)
{
    ...
    {
        // 获取主屏幕的尺寸
        // int currentScreenWidth = QApplication::desktop()->width(); // 过时的方法
        // int currentScreenHeight = QApplication::desktop()->height(); // 过时的方法
        int currentScreenWidth = QGuiApplication::primaryScreen()->geometry().width();
        int currentScreenHeight = QGuiApplication::primaryScreen()->geometry().height();
...
void Widget::save_default_rect_info()
{
    ...
    // 获取主屏幕的宽度和高度
    QScreen *primaryScreen = QGuiApplication::primaryScreen();
    int currentScreenWidth = primaryScreen->geometry().width(); // 获取主屏幕宽度
    int currentScreenHeight = primaryScreen->geometry().height() - 50; // 获取主屏幕高度，减去50
    ...
    // 打印当前屏幕数量
    qDebug().nospace() << __FILE__ << "(" << __LINE__ << "):" << __FUNCTION__ << " screen count=" << QGuiApplication::screens().count();
